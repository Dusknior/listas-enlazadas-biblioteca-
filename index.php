...
?>
	...
	<form  action="index.php" method="get" class="met1">
			...
			<fieldset >
				<input class="registro" type="text" name="CANT" placeholder="cantidad a agregar">
			</fieldset>
			...
			<fieldset>
				<select class="registro" name="funcion" id="funciones">
					...
					<option value="nlap">names de libros en años pares</option>
 ...
 <?php  
	 ...
	 if (isset($_GET['buscar'] )) {
	 	if ($_GET['funcion'] == "be") {
	 		...
	 		echo "<br><hr>".$_SESSION['lista']->DetalleEditorial($_GET["IDE"]);

	 	}

	 	if ($_GET['funcion']== "bl") {
	 		
	 		echo "<br><hr>".$_SESSION['lista']->Detallelibro($_GET["IDE"],$_GET["IDL"]);
	   	
	 	}

	 	if ($_GET['funcion']== "ee") {
	 		 
	   	$resul= $_SESSION['lista']->EliminarEditorial($_GET["IDE"]);
	   		if ($resul) {
	 	...
	 	if ($_GET['funcion']== "nlap") {
	 		 
	   	$result = $_SESSION['lista']->namesLibrosAnosPares();
	   	echo "<br><hr>Los libros son: ".$result;
	 	}
